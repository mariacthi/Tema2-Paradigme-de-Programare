...
toRegion region = case region of
    ...
    Complement r -> S.complement (toRegion r)
    Union r1 r2 -> S.union (toRegion r1) (toRegion r2)
    Intersection r1 r2 -> S.intersection (toRegion r1) (toRegion r2)
    Transform transformation r -> S.applyTransformation (toTransformation transformation) (toRegion r)
...
fuseTransformations = foldr fuse []
  where
    fuse transformation acc = case acc of
        [] -> [transformation]
        prev:rest -> case (prev, transformation) of
            (Translation tx1 ty1, Translation tx2 ty2) ->
                Translation (tx1 + tx2) (ty1 + ty2) : rest
            (Scaling factor1, Scaling factor2) ->
                Scaling (factor1 * factor2) : rest
            _ -> transformation : acc
{-
    *** TODO ***
...
optimizeTransformations region = case region of
    -- verifica recursiv regiunile, incercand sa le simplifice
    Transform t r ->
        let optimized = optimizeTransformations r
        in case optimized of
            -- combina cele doua transformari daca sunt consecutive, altfel
            -- foloseste regiunea optimizata
            Transform t' r' -> simplifyTransformations (Transform (combineTransformations [t, t']) r')
            _ -> simplifyTransformations (Transform t optimized)
    Union r1 r2 ->
    ...
    Complement r ->
        let optimized = optimizeTransformations r
        in case optimized of
            Transform t r' -> simplifyTransformations (Transform t (Complement r'))
            _ -> Complement optimized
    _ -> region

simplifyTransformations :: RegionAST -> RegionAST
simplifyTransformations (Transform t r) =
    let transformations = fuseTransformations $ reduceCombine t
    in case transformations of
        -- dacă există o singură transformare rezultată din simplificare,
        -- se reconstruiește regiunea folosind acea transformare, altfel
        -- le combina pe toate
        [t'] -> Transform t' r
        _ -> Transform (Combine transformations) r
  where
    -- creaza o lista de transformari simple pentru a putea fi trimise ca
    -- argument functiei fuseTransformations
    reduceCombine (Combine ts) = concatMap reduceCombine ts
    reduceCombine t = [t]
