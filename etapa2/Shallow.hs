...
inside = \p -> ($ p)
-- aplica argumentul p (Point) functiei specificate de $
...
fromPoints = \ps p -> any (\q -> q == p) ps
-- iterez prin lista de Point ps si verific daca un element din lista (q)
-- este egal cu punctul cerut (p)
...
-- verific toate punctele din coltul de stanga sus la cel din dreapta jos (linie cu linie)
...
promoteUnary = \f pointed point -> f . pointed $ point
...
bfs start expand = bfs' [(start, 0)] S.empty
  where
    bfs' [] _ = [] -- daca nu mai exista noduri de vizitat, returneaza lista vida
    bfs' ((node, level):rest) visited
      | S.member node visited = bfs' rest visited
      -- daca nodul face parte din cele vizitate, face bfs pe restul listei
      | otherwise =
          let neighbours = filter (`S.notMember` visited) $ expand node
              -- afla care sunt vecinii nevizitati ai nodului
              newVisited = S.insert node visited
              -- marcheaza ca vizitat nodul
              newRest = rest ++ zip neighbours (repeat (level + 1))
              -- vecinii sunt adaugati in coada pe nivelul urmator
          in (node, level) : bfs' newRest newVisited
...
regionAvoidingBfs start region = bfs start (expand region)
  where
    ...
    -- codificare pentru cele patru modalitati de deplasare
    expand region (x, y) = filter (not . (`inside` region)) $
                           map (\(dx, dy) -> (x + dx, y + dy)) directions
    -- aflu vecinii in cele patru directii care nu apartin regiunii date