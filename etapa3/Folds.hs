...
instance Show TransformationAST where
    ...
    -- desface transformarile combinate in cele doua cazuri de mai sus
{-
    *** TODO ***
...
showRegionAST indentLevel (R regionShape) =
    ...
    case regionShape of
        ...
        Complement region -> "~\n" ++ showRegionAST (indentLevel + 2) region
        Union region1 region2 -> "+\n" ++
            showRegionAST (indentLevel + 2) region1 ++ "\n" ++
            showRegionAST (indentLevel + 2) region2
        Intersection region1 region2 -> "*\n" ++
            showRegionAST (indentLevel + 2) region1 ++ "\n" ++
            showRegionAST (indentLevel + 2) region2
        Transform transformation region -> show transformation ++
            "\n" ++ showRegionAST (indentLevel + 2) region
    -- respecta cerintele de afisare de mai sus si indenteaza atunci cand se
    -- apeleaza in continuare functia pe regiuni
...
instance Functor TransformationShape where
    ...
    fmap f (Combine values) = Combine (map f values)
    -- aplica f pe toate valorile din Combine
